...
**M5 (engineering half) — Hardening: shipped.** Org administration is live: member management with role changes and last-admin protection, and the append-only audit log surfaced as an admin screen. A **one-click sample project** seeds the synthetic Northbridge Mutual corpus so a design partner sees the full loop before uploading a real document. **PDF export** landed for all four deliverable types (PyMuPDF — no system deps, closing the M4 deferral). Hardening: per-IP rate limiting on auth/upload/pipeline surfaces, security headers, an upload extension allowlist, and per-version feedback capture (the correction flywheel's simplest instrument). Deliberately *not* built: Stripe billing (design partners pilot free; a half-integrated payment flow is worse than a plan field) and load testing at 500-doc scale (needs a deployed environment). **The remaining M5 work is human:** recruit 3–5 design partners, run a real engagement, put a generated deck in front of a real executive — and resolve the employment-IP question before the first external user (docs/01 §6).
